...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50">
      ...
      <div className="hidden md:block bg-royal-blue text-cream text-xs md:text-sm">
      ...
      <div className="bg-cream/95 glass-frosted border-b border-royal-blue/10 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-3 md:py-4 flex items-center justify-between gap-3 md:gap-4">
          ...
          <Link href="/" className="flex items-center gap-2 flex-shrink-0 min-w-fit">
            <div className="w-9 h-9 bg-royal-blue rounded-lg flex items-center justify-center flex-shrink-0">
              <span className="text-cream font-bold text-xs leading-none text-center">LS</span>
            </div>
            <div className="hidden sm:block">
              <span className="font-playfair font-bold text-sm md:text-lg text-royal-blue leading-tight block">
                Linkin Studio
              </span>
        ...
        {/* Mobile Navigation */}
        {isMenuOpen && (
          <div className="md:hidden border-t border-royal-blue/10 bg-cream py-3 px-4 space-y-2">
            <Link href="/" className="block py-2.5 px-3 rounded-lg text-charcoal hover:bg-light-gray hover:text-royal-blue transition-colors font-medium text-sm" onClick={() => setIsMenuOpen(false)}>Home</Link>
            
            <div className="py-2 px-3 border-t border-border-gray mt-2 pt-3">
              <p className="text-xs font-bold text-royal-blue/50 uppercase tracking-widest mb-2">Photography</p>
              <Link href="/wedding-films" className="block py-1.5 pl-2 text-sm text-charcoal hover:text-royal-blue transition-colors" onClick={() => setIsMenuOpen(false)}>Wedding Films</Link>
              <Link href="/photography" className="block py-1.5 pl-2 text-sm text-charcoal hover:text-royal-blue transition-colors" onClick={() => setIsMenuOpen(false)}>Photography</Link>
            </div>

            <div className="py-2 px-3 border-t border-border-gray">
              <p className="text-xs font-bold text-royal-blue/50 uppercase tracking-widest mb-2">Services</p>
              <Link href="/bridal-wear" className="block py-1.5 pl-2 text-sm text-charcoal hover:text-royal-blue transition-colors" onClick={() => setIsMenuOpen(false)}>Bridal Wear</Link>
              <Link href="/accessories" className="block py-1.5 pl-2 text-sm text-charcoal hover:text-royal-blue transition-colors" onClick={() => setIsMenuOpen(false)}>Accessories</Link>
              <Link href="/#passport-photos" className="block py-1.5 pl-2 text-sm text-charcoal hover:text-royal-blue transition-colors" onClick={() => setIsMenuOpen(false)}>Passport Photos</Link>
            </div>

            <Link href="/gallery" className="block py-2.5 px-3 rounded-lg text-charcoal hover:bg-light-gray hover:text-royal-blue transition-colors font-medium text-sm border-t border-border-gray mt-2 pt-3" onClick={() => setIsMenuOpen(false)}>Gallery</Link>
            
            <a href="#contact" className="block py-2.5 px-3 rounded-lg text-charcoal hover:bg-light-gray hover:text-royal-blue transition-colors font-medium text-sm" onClick={() => setIsMenuOpen(false)}>Contact</a>
            
            <a
              href="https://wa.me/17785397039"
              target="_blank"
              rel="noopener noreferrer"
              className="block w-full py-3 px-4 bg-royal-blue text-cream rounded-lg font-semibold text-center text-sm mt-3 hover:bg-royal-blue/90 transition-colors"
